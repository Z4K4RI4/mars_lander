...
class Fleet:
    ...
    def crossover(self):
        ...
        for parent1 in self.selection:
            ...
            #     parent2 = np.random.choice(self.selection, size=1)[0]
            rand_cross_factor = np.random.random()  # generate a random cross_factor
            ...
            # child1_powers = parent1.powers[:rand_idx] + parent2.powers[rand_idx:]
            # child1_rotations = (
            #     parent1.rotations[:rand_idx] + parent2.rotations[rand_idx:]
            # )
            # child2_powers = parent2.powers[:rand_idx] + parent1.powers[rand_idx:]
            # child2_rotations = (
            #     parent2.rotations[:rand_idx] + parent1.rotations[rand_idx:]
            # )
            child1 = Simulator(
                x=self.init_x,
                y=self.init_y,
    ...
    def new_generation(self, surface):
        ...
        self.select_graded()
        self.crossover()
        # self.mutation(flip_rate=80)

        # init_x, init_y, init_h_speed, init_v_speed = self.selection[0].vector_hist[0]

        # pool = ThreadPool(10)
        for sim in self.generations[-1]:
            # pool.apply_async(Simulator.land_a_lander, (sim, surface))
